...
nlcd <- nlcd %>% crop(as_Spatial(acty))
nlcd <- mask(nlcd, acty) %>% project('epsg: 3857')
...
ggplot() + geom_sf(data=nlcd.poly, aes(fill=`Color`), linewidth=0) +
  ...
  coord_sf(expand=F) +
  theme_void() + 
  labs(title = str_c('<span style="">Land Cover in the <br /><b style="font-size: 43pt; color: gray30">Albany County</b></span><br />',
                     '2019 National Land Cover Dataset<br />Andy Arthur, 2/4/23'),
      )  +
  theme(
    text= element_text(family='Roboto Condensed',size=14),
    plot.title=ggtext::element_textbox(hjust=0, halign=0, face='bold', height=0, size=22),
