...
library(lubridate)
days90 <- wx %>% filter(DATE > today() %m-% days(92))
...
ggplot(days90, aes(DATE, DEP, fill=DEP)) + geom_hline(yintercept = 0, size=2, color="#413e4f") + geom_col() + 
  ...
  theme_minimal() +
  labs(title = 'Temperature Departure from Normal, Past 90 Days',
...
ggsave('/tmp/Depature.jpg', width=1920, height=1080, units='px', dpi=150)
ggsave('/tmp/Departure.svg', width=1920, height=1080, units='px', dpi=150, device = grDevices::svg)

ggplot(days90, aes(DATE, MAX)) + geom_hline(color="#413e4f", yintercept = 0, size=2) + geom_col(aes(fill=MAX),alpha=0.95) + 
  ...
  guides(fill = guide_colourbar(barwidth = 60, barheight = 0.5,ticks = T, size=0.3,
                                frame.colour = "#333333", label.position = "bottom"))

ggsave('/tmp/High Temp.jpg', width=1920, height=1600, units='px', dpi=150)
ggsave('/tmp/High Temp.svg', width=1920, height=1600, units='px', dpi=150, device = grDevices::svg)

ggplot(days90, aes(DATE, MIN)) + geom_hline(color="#413e4f", yintercept = 0, size=2) + geom_col(aes(fill=MIN),alpha=0.95) + 
  scale_fill_gradientn(name = "", colors = c("pink", "violet", "purple", "blue", "cyan", "green", "yellow", "orange", "red", "darkred"), space = "Lab",
                       na.value = "grey50", guide = "colourbar", aesthetics = "fill", limits = c(-15,115), 
                       breaks=seq(-30,130,10)) +
  scale_x_date(date_breaks = 'week', date_minor_breaks = 'day', date_labels = '%b %-d', expand=c(0,0)) +
  ...
  theme_minimal() +
  labs(title = 'Low Temperature, Last 90 Days',
       y = "",
       x = "",
       caption='National Weather Service, CF-6 Reports',
       tag=paste('Andy Arthur,', format(Sys.Date(), format="%-m/%-d/%y")),
...
ggsave('/tmp/Low Temp.jpg', width=1920, height=1600, units='px', dpi=150)
ggsave('/tmp/Low Temp.svg', width=1920, height=1600, units='px', dpi=150, device = grDevices::svg)
