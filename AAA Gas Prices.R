...
states %>% 
  ...
  ggplot() +
  geom_sf(aes(fill=Regular), size=0.3, color='black') + 
  ...
  labs(title = str_c('<span style="color: #cccc00">Regular Gas Price</span> by State as of ',  format( now(),  format="%-B %-d, %Y") ),
       y = "",
       x = "",
...
states %>% 
  inner_join(gas, by=c('NAME'='State')) %>%
  ggplot() +
  geom_sf(aes(fill=Diesel), size=0.3, color='black') + 
  ggsflabel::geom_sf_label_repel(aes(label=str_c(STUSPS,'\n',scales::dollar(Diesel, accuracy = 0.01)), geometry=geometry),
                                 size=2.5, point.size = NA, fill='white', box.padding = 0.2, label.padding = unit(0.2, "lines"), family='Noto Sans', max.overlaps=NA ) +
  scale_fill_viridis_c(option='C', label=scales::dollar_format()) +
  labs(title = str_c('<span style="color: darkgreen">On-Road Diesel Price</span> by State as of ',  format( now(),  format="%-B %-d, %Y") ),
       y = "",
       x = "",
       caption='https://gasprices.aaa.com/state-gas-price-averages/',
       tag=paste('Andy Arthur,', format(Sys.Date(), format="%-m/%-d/%y")),
