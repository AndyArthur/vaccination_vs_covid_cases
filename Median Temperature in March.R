...
climate <- read_csv("https://www.ncdc.noaa.gov/cag/county/time-series/NY-001-tavg-1-4-1895-2021.csv?base_prd=true&begbaseyear=1895&endbaseyear=2021", 
...
ggplot(climate, aes(x=Date, y=Value)) + geom_segment(aes(xend=Date, 
  ...
  geom_hline(yintercept = median(climate$Value)) + labs(y=paste('Median Temperature in ',month,' (F)'), x='Year') +
 ...
 annotate('text', x=as.Date('1900-01-01'),y=Inf, hjust=0, vjust=1, label=get_formula(lm(Value ~ Date, data=climate))[1] ) +
  labs(title=paste('Median',month,'Temperature for Albany'),
       subtitle=paste('In recent years, Albany\'s',month,'temperature has risen noticably above the historical average of',round(mean(climate$Value),1),'degrees...'),
...
ggsave(paste('/tmp/median-',month,'-temp.svg',sep=''), width=1920, height=1080, units='px', dpi=150, device = grDevices::svg)
ggsave(paste('/tmp/median-',month,'-temp.jpg',sep=''), width=1920, height=1080, units='px', dpi=150)
