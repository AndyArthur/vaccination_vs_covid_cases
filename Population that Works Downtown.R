...
ny_lodes_od %>% filter(w_tract == '36001001100') -> dt
...
dt_commuters <- cap_adults %>%
  left_join(dt, by = c("GEOID" = "h_tract")) %>%
...
ggplot(dt_commuters) +
  ...
  coord_sf(expand = F) +
  labs(
    title = str_c('<span style="color: navy; font-size: 35pt;">Population that Works Downtown</span><br />
                  Area includes Empire State Plaza and Capitol Complex, State Street, SUNY HQ.'),
    y = "",
    x = "",
    caption = "",
