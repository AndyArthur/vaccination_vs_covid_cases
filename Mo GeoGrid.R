...
df <- get_decennial(geography = "county", state='or',variables = "P1_001N", 
                     year = 2020, cache=TRUE, geometry = T) %>%
  st_set_crs('4326')

tmp <- calculate_grid(shape = df, grid_type = "regular", seed = 2)
df_hex <- assign_polygons(df, tmp) 
df_hex = st_as_sf(df_hex)

df_hex <- df_hex %>% mutate(NAME = str_replace(NAME, ' County, Oregon',''),
                            NAME = str_replace(NAME, ', Oregon',''))

ggplot(df_hex) + geom_sf(aes(fill=value), color='white', size=1) + 
  ...
  theme(
    ...
    axis.title.y.left = element_text(angle=90, face='bold',size=36),
