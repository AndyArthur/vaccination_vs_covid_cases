...
library(terra)
library(ggtext)
#data <- GetCDLData(aoi = '36', year=2022, type='f', save_path = '/tmp/22nycrop.tif')
data <- rast('/tmp/22nycrop.tif') == 229
...
nygrid %>% cbind(pumpkin.cover) %>%
  ...
  geom_sf(data=nyco, fill=NA, color='darkorange', linetype='dotted') +
  scale_fill_steps(low='white', high='darkorange', trans='log10', limits=c(1,180), na.value = 'white', n.breaks=8, labels=scales::label_comma(), name='Acres') +
  coord_sf(expand=F, crs=3857) +
  theme_void() +
  labs(title = str_c('<span style="font-size: 58pt; color: lightyellow">Pumpkins 🎃</span><br /><br />Grown in New York State (2022)</em>'),
       y = "",
       x = "",
  ...
  theme(
    ...
    plot.background = element_rect(fill = "darkorange", color="darkorange"),
  ...
  guides( fill = guide_legend(
    title.theme = element_text(face='bold', size=18, color='lightyellow', hjust = 0.5),
    label.theme = element_text(color='white', size=14),                       
                              label.hjust = 1, 
    override.aes = list(size=6, shape='diamond' )))

fn <- 'Pumpkins_Grown_in_NYS'
