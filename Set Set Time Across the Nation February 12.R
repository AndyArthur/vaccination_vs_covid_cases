...
ss = getSunlightTimes(
  data = bind_cols(usco.centroid, date=rep(Sys.Date(), nrow(usco.centroid))),
  keep = c('sunset')
) %>%
  bind_cols(GEOID = usco.centroid$GEOID, ., offset=usco.centroid$`timezones-data_offset`) %>%
...
usco %>%
  left_join(ss) %>%
  mutate(sunset = cut(sunset + offset + 5*60*60, breaks = seq(as.POSIXct('2023-02-05 00:00'), as.POSIXct('2023-02-05 24:00'), '10 mins'),
                      labels = format(seq(as.POSIXct('2023-02-05 00:00'), as.POSIXct('2023-02-05 23:50'), '10 mins'),'%I:%M %P'))
         ) %>%
  ...
  labs(title = str_c('<span style="font-size: 35pt; color: ',rclr[3],'">Local Time Sunset</span> <span style="font-size: 30pt">on</span> ',
                     '<span style="font-size: 35pt; color: ',rclr[7],'">February 5, 2023</span>',
                     '<br />Approximate local time when the sun sets tonight.'),
       y = "",
       x = "",
