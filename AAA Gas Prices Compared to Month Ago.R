...
rm(list=ls())
url <- 'https://gasprices.aaa.com/top-trends/?filter%5Baverage%5D=month&filter%5Bregion%5D=st&filter%5Bby%5D=least&filter%5Bfuel%5D=unleaded'
...
states %>% 
  ...
  labs(title = str_c('Increase in Price for a Gallon of <span style="color: #cccc00">Regular Gas</span> Since ',  format( now() - months(1),  format="%-B %-d, %Y") ),
       y = "",
       x = "",
